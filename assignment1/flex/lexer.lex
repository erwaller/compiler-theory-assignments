...
main()
{
	...
	while ((ret = yylex()) != TOKEOF) {
	    printf("%d\t%s", line_number, debug_token(ret));
	    switch (ret) {
	        case IDENT:
    	        printf("\t%s", yytext);
    	        break;
